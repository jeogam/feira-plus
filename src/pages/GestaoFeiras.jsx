...
const GestaoFeiras = () => {
  ...
  const carregarFeiras = async () => {
    ...
    try {
      ...
      setMensagemModal(
        "Não foi possível carregar as feiras. Verifique se o servidor está rodando."
      );
  ...
  const feirasFiltradas = feiras.filter(
    (feira) =>
  ...
  return (
    <div className="container-fluid p-4">
                ...
                {loading ? (
                  ...
                  feirasFiltradas.map((feira) => (
                    <tr key={`${feira.tipo}-${feira.id}`}>
                      <td className="ps-4">
                        <img
                          src={feira.foto?.trim() ? feira.foto.trim() : PLACEHOLDER_IMG}
                          alt="Foto da feira"
                          className="rounded"
                      ...
                      <td className="fw-bold text-dark">{feira.nome}</td>
                      <td>{feira.local}</td>
                      <td>
                      ...
                      <td className="text-end pe-4">
                        <button
                          ...
                          onClick={() => handleEditar(feira)}
                        >
                          <i className="bi bi-pencil"></i>
                        </button>
                        <button
                          className="btn btn-sm btn-outline-danger"
                          onClick={() => handleConfirmarExclusao(feira)}
                        >
      ...
      <FeiraFormModal
        ...
        expositoresDisponiveis={expositores}
        loadingExpositores={loadingExpositores}
      />
